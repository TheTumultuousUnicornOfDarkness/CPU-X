...
static void labels_setname(Labels *data)
{
	...
	asprintf(&data->tab_cpu[NAME][EXTFAMILY],       _("Disp. Family"));
	asprintf(&data->tab_cpu[NAME][MODEL],           _("Model"));
	asprintf(&data->tab_cpu[NAME][EXTMODEL],        _("Disp. Model"));
	...
	for(i = GPU1VENDOR; i < LASTGRAPHICS; i += GPUFIELDS)
	{
		...
		asprintf(&data->tab_graphics[NAME][GPU1VBIOS       + i], _("VBIOS Version"));
...
static void sighandler(int signum)
{
	...
	/* Print the backtrace */
	fprintf(stderr, colorized_msg(BOLD_RED, _("\nOops, something was wrong! %s has received signal %d (%s) and has crashed."),
		PRGNAME, signum, strsignal(signum)));
